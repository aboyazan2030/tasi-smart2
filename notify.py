...
تدعم: السوق السعودي + السوق الأمريكي
"""
import os
import requests
...
def build_alert_message(top_stocks, date_str: str) -> str:
    buy_signals  = [s for s in top_stocks if s["التصنيف"] in (
        "🚀 اختراق صاعد", "📦 تجميع هادئ", "🔄 ارتداد من تشبع بيع")]
    sell_signals = [s for s in top_stocks if s["التصنيف"] == "⚠️ تصريف/توزيع"]

    lines = [f"<b>🇸🇦 تقرير تاسي الذكي – {date_str}</b>\n"]
...
def build_alert_message_us(top_stocks, date_str: str) -> str:
    buy_signals  = [s for s in top_stocks if s["التصنيف"] in (
        "🚀 اختراق صاعد", "📦 تجميع هادئ", "🔄 ارتداد من تشبع بيع")]
    sell_signals = [s for s in top_stocks if s["التصنيف"] == "⚠️ تصريف/توزيع"]

    lines = [f"<b>🇺🇸 تقرير السوق الأمريكي الذكي – {date_str}</b>\n"]
...
def send_daily_alerts_us(top_stocks, date_str: str):
    msg  = build_alert_message_us(top_stocks, date_str)
    sent = False
    if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
        sent = send_telegram(msg)
    if EMAIL_TO and EMAIL_FROM and EMAIL_PASSWORD:
        plain = msg.replace("<b>", "").replace("</b>", "")
        html  = msg.replace("\n", "<br>")
        send_email(f"🇺🇸 تقرير السوق الأمريكي – {date_str}", html)
        sent = True
    if not sent:
        print("تحذير: لم يُرسَل أي إشعار للسوق الأمريكي")
